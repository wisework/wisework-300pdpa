...
class ChooseRejectTypeModal extends StatefulWidget {
  ...
  final Function(UpdatedReturn<RejectTypeModel> rejectType) onUpdated;
...
class _ChooseRejectTypeModalState extends State<ChooseRejectTypeModal> {
  ...
  void _initialData() {
    rejectTypes = widget.rejectTypes
        .where((rejectType) => rejectType.status != ActiveStatus.inactive)
        .map((rejectType) => rejectType)
        .toList();
  ...
  void _selectRejectType(RejectTypeModel rejectType) {
    final selectIds = selectRejectTypes.map((selected) => selected.id).toList();

    setState(() {
      if (selectIds.contains(rejectType.id)) {
        selectRejectTypes = selectRejectTypes
            .where((selected) => selected.id != rejectType.id)
            .toList();
      } else {
        selectRejectTypes = selectRejectTypes
      ...
      children: <Widget>[
        Padding(
          ...
          child: CustomCheckBox(
            value: selectIds.contains(rejectType.id),
            onChanged: (_) {
              _selectRejectType(rejectType);
            },
          ),
        ),
        Expanded(
      ...
      onPressed: () async {
        await context.push(MasterDataRoute.createRejectType.path).then((value) {
          if (value != null) {
            ...
            _selectRejectType(updated.object);
