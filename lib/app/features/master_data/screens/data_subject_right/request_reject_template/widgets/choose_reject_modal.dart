...
class ChooseRejectModal extends StatefulWidget {
  const ChooseRejectModal({
    super.key,
    required this.initialRejects,
  ...
  final List<RejectTypeModel> initialRejects;
  final List<RejectTypeModel> rejects;
  final Function(List<RejectTypeModel> rejects) onChanged;
  final Function(UpdatedReturn<RejectTypeModel> reject) onUpdated;
  final String language;

  @override
  State<ChooseRejectModal> createState() => _ChooseRejectModalState();
}

class _ChooseRejectModalState extends State<ChooseRejectModal> {
  late List<RejectTypeModel> rejects;
  late List<RejectTypeModel> selectRejects;
  ...
  void _initialData() {
    rejects = widget.rejects
        ...
        .map((reject) => reject)
        .toList();
    selectRejects = widget.initialRejects
        .where((reject) => reject.status != ActiveStatus.inactive)
        .map((reject) => reject)
        .toList();
  }
  ...
  void _selectReject(RejectTypeModel reject) {
    final selectIds = selectRejects.map((selected) => selected.id).toList();

    setState(() {
      if (selectIds.contains(reject.id)) {
        selectRejects = selectRejects
            .where((selected) => selected.id != reject.id)
            .toList();
      } else {
        selectRejects = selectRejects.map((selected) => selected).toList()
          ..add(reject);
      }
    });

    widget.onChanged(selectRejects);
  }
  ...
  Row _buildCheckBoxListTile(
    ...
    final selectIds = selectRejects.map((category) => category.id).toList();
      ...
      onPressed: () async {
        await context.push(MasterDataRoute.createRejectType.path).then((value) {
          if (value != null) {
            ...
            rejects = rejects..add(updated.object);
